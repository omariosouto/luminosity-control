app.controller('funcionariosController', ['$scope', '$location', 'funcionarios', 'funcionariosAPI', function($scope, $location, funcionarios, funcionariosAPI){
	...
	var userToken = JSON.parse(window.localStorage['loginToken']);
	...
	$scope.adicionarFuncionario = function(manipularFuncionario) {
		...
		funcionariosAPI.addFuncionario(manipularFuncionario).then(function(data){
			console.log(data);
	...
	$scope.apagarFuncionario = function(funcionarios) {
		...
		for (var i = funcionariosSelecionados.length - 1; i >= 0; i--) {
			...
			funcionariosAPI.deleteFuncionario(funcionariosSelecionados[i].FUN_INT_ID).then(function(data){
				console.log(data);
				carregarFuncionarios();
	...
	$scope.alterarFuncionario = function(manipularFuncionario) {
		...
		manipularFuncionario.userToken = userToken;

		console.log('--------------------------------------------------');
		console.log(manipularFuncionario.userToken);
		console.log('--------------------------------------------------');

		funcionariosAPI.updateFuncionario(manipularFuncionario).then(function(data){
